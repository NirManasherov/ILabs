...
{//handles assignment to self    
   o_.IncreaseCount();//in the right-side string
...
String::string_handle_ty *String::AllocStringIMP(const char *src)
{
    ...
    string_handle_ty *ret_str_handler = static_cast<string_handle_ty *>
              (operator new(offsetof(string_handle, m_str_begining) + len + 1));
