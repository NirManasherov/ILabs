...
void SetAddrStruct(struct sockaddr_in *addr_)
{
    assert(NULL != addr_);
    
    addr_->sin_family = AF_INET;
    addr_->sin_port = htons(1025); /*for big/little endien correction*/
    addr_->sin_addr.s_addr = inet_addr("127.0.0.1"); /*IP for local host*/
}
...
int Recieve(int sockfd_, void *buf_, size_t len_, int flags_)
{
...
ssize_t SendMsg(int sockfd_, const void *buf_, size_t len_, int flags_)
{
